...
function Collision( coord ){
	for( i = 0; i < vPoints.length; i++ ){
		r = vPoints[i];
		if( ( coord[0] >= parseInt(r.x) && coord[0] <= parseInt(r.x) + parseInt(r.w) ) && ( coord[1] >= parseInt(r.y) && coord[1] <= parseInt(r.y) + parseInt(r.h) ) ){
			vPoints[i].drag = true;
			return true;
		}
	}
...
function Loop(context) {
	vPoints[0].x = rForm[0].value; vPoints[0].y = rForm[1].value;
	vPoints[1].x = bForm[0].value; vPoints[1].y = bForm[1].value;
	vPoints[2].x = gForm[0].value; vPoints[2].y = gForm[1].value;
